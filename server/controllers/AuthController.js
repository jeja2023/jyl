...
class AuthController {
    ...
    static async smsLogin(ctx) {
        ...
        // 使用 UserService 处理用户逻辑
        const { user, isNewUser } = await UserService.findOrCreateByPhone(phone, { nickname });
    ...
    static async wechatLogin(ctx) {
        ...
        try {
            ...
            // 使用 UserService 处理用户逻辑
            const { user, isNewUser } = await UserService.findOrCreateByWechat(openid, unionid, userInfo);
    ...
    static async wechatPhoneLogin(ctx) {
        ...
        try {
            ...
            // 使用 UserService 处理用户逻辑
            const { user, isNewUser } = await UserService.handleWechatPhoneLogin(phone, openid, unionid);

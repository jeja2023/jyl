...
const HealthRecord = sequelize.define('HealthRecord', {
    ...
    reportImage: {
        ...
        get() {
            const rawValue = this.getDataValue('reportImage');
            if (!rawValue) return [];
            try {
        ...
        set(value) {
            this.setDataValue('reportImage', JSON.stringify(value || []));
        }
    },
    ultrasoundImage: {
        type: DataTypes.TEXT,
        ...
        get() {
            const rawValue = this.getDataValue('ultrasoundImage');
            if (!rawValue) return [];
            try {
                const result = JSON.parse(rawValue);
                return Array.isArray(result) ? result : [result];
        ...
        set(value) {
            this.setDataValue('ultrasoundImage', JSON.stringify(value || []));
        }
    }
}, {
    timestamps: true,

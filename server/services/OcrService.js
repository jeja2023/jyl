...
class OcrService {
    ...
    static parseIndicators(textDetections) {
        ...
        const patterns = [
            ...
            { key: 'FT3', regex: new RegExp(`(?:FT3|${k('游离T3')}|${k('游离三碘')}|${k('游离三碘甲状腺原氨酸')})[:：\\s]*([0-9]+\\.?[0-9]*)`, 'i') },
            ...
            { key: 'TPOAb', regex: new RegExp(`(?:TPO-?Ab|${k('TPO抗体')}|${k('抗甲状腺过氧化物酶抗体')}|${k('甲状腺过氧化物')})[:：\\s]*([0-9]+\\.?[0-9]*)`, 'i') },
            { key: 'TGAb', regex: new RegExp(`(?:TGAb|A-TG|${k('TG抗体')}|${k('抗甲状腺球蛋白抗体')}|${k('甲状腺球蛋白')})[:：\\s]*([0-9]+\\.?[0-9]*)`, 'i') },
